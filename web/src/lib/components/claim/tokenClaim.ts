...
import {formatError} from '$lib/utils/debug';
import {estimateGasPrice} from '$lib/utils/eth/gas';
import {BaseStore} from '$lib/utils/stores/base';
import {rebuildLocationHash} from '$lib/utils/url';
import {account, connection, contracts, network} from '$lib/web3';
...
async function claim() {
	...
	contracts.execute(async ({contracts, publicClient, account, connection}) => {
		let ethBalance: bigint;
		let tokenBalance: bigint;
		let nonce: number;
		let estimate: bigint;
		try {
			ethBalance = await publicClient.getBalance(claimWallet);
			tokenBalance = await contracts.TestTokens.read.balanceOf([claimWallet.address]);
			if (tokenBalance == 0n) {
				// TODO
			}
			nonce = await publicClient.getTransactionCount({address: claimWallet.address, blockTag: 'latest'});
